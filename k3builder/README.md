...
```sh
make docker-build docker-push IMG=<some-registry>/k3builder:tag
```
...
```sh
make deploy IMG=<some-registry>/k3builder:tag
```
...
```sh
make build-installer IMG=<some-registry>/k3builder:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/k3builder/<tag or branch>/dist/install.yaml
```
